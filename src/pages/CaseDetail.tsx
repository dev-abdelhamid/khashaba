...
import { ArrowLeft, Star, Calendar, Clock, Users, Heart, CheckCircle, Quote, Facebook, Instagram, ZoomIn, X } from 'lucide-react';
...
const BeforeAfterSlider: React.FC<{ beforeImage: string; afterImage: string; altText: string }> = ({
  ...
  return (
    <div className="relative flex items-center justify-center">
      <div
        ref={sliderRef}
        className="relative rounded-lg w-full min-h-[300px] max-h-[550px] overflow-hidden aspect-[16/9] shadow-xl bg-dental-dark cursor-ew-resize"
      ...
      >
        ...
        <Button
          size="sm"
          className="absolute top-4 left-4 bg-dental-gold/90 text-white px-3 py-1.5 rounded-full text-sm font-medium z-30 hover:bg-dental-darkGold"
        ...
        <Button
          size="sm"
          className="absolute top-4 right-4 bg-dental-gold/90 text-white px-3 py-1.5 rounded-full text-sm font-medium z-30 hover:bg-dental-darkGold"
      ...
      <div ref={ref} className="relative max-w-5xl mx-auto pb-16">
        ...
        {data.map((item, index) => (
          <div
            key={index}
            className={cn(
              "flex flex-col md:flex-row items-center mb-8 sm:mb-12 gap-4 sm:gap-6",
              index % 2 === 0
                ? isRTL
                  ? "md:flex-row-reverse"
                  : "md:flex-row"
          ...
          >
            <div
              className={cn(
                ...
                isRTL && index % 2 === 0 ? "md:pr-8" : "md:pl-8",
                !isRTL && index % 2 === 0 ? "md:pl-8" : "md:pr-8"
              )}
            >
              <h3 className="text-base sm:text-lg font-semibold text-dental-dark dark:text-white mb-2">
                {item.step}
              </h3>
              <p className="text-dental-dark dark:text-white text-sm leading-relaxed">
                {item.description}
              </p>
            </div>
            <div className="flex items-center justify-center w-10 h-10 bg-dental-gold rounded-full shadow-md z-10 md:absolute md:left-1/2 md:-translate-x-1/2">
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-dental-gold/5 to-dental-dark/5 font-amiri text-sm leading-relaxed" dir={isRTL ? 'rtl' : 'ltr'}>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 ">
          ...
          >
                  ...
                  <SwiperSlide key={idx}>
                    ...
                    >
                      <LazyImage
                        ...
                        className="w-full h-[280px] sm:h-[380px] lg:h-[480px] object-cover rounded-xl"
                        loading="lazy"
                      />
            ...
            <div className="w-full md:w-1/2 flex flex-col justify-center gap-6 text-center">
              ...
              <div className={cn('flex gap-3 justify-center md:justify-center', isRTL ? 'flex-row-reverse' : '')}>
                <Button
                  variant="outline"
                  size="icon"
                  className="rounded-full bg-dental-dark/10 dark:bg-dental-dark/20 text-dental-dark dark:text-white border-dental-gold/20 hover:bg-dental-gold hover:text-white transition-all duration-200"
                ...
                >
                  <a
                    href={`https://www.tiktok.com/share?url=${encodeURIComponent(shareUrl)}&text=${encodeURIComponent(shareTitle)}`}
                    target="_blank"
                ...
                >
                  <a href={caseItem.instagramLink} target="_blank" rel="noopener noreferrer">
      ...
      <section id="case-details" className="py-6  ">
        ...
        <motion.div
          className=" container flex flex-col  md:flex-row justify-center w-full   gap-6  items-center align-center w-full mx-auto  mt-10"
          variants={containerVariants}
          initial="hidden"
          animate="visible"
        >
          <div  className="w-full md:w-1/3 flex flex-col gap-6 container flex flex-col  md:flex-row justify-center w-full " >
            <motion.div
              className="bg-dental-dark/10 dark:bg-dental-dark/20 rounded-xl p-8 shadow-md sticky "
              variants={containerVariants}
              initial="hidden"
              animate="visible"
            >
              ...
              <ul className="space-y-4 text-xs text-dental-dark dark:text-white">
                <li className="flex items-center gap-2 justify-center">
                  <Calendar className="w-4 h-4 text-dental-gold" />
                  <span>{isRTL ? 'تاريخ العلاج: ' : 'Treatment Date: '}{caseItem.date[language]}</span>
                </li>
                <li className="flex items-center gap-2 justify-center">
                  <Users className="w-4 h-4 text-dental-gold" />
                  <span>{isRTL ? 'عمر المريض: ' : 'Patient Age: '}{caseItem.patientAge[language]}</span>
                </li>
                <li className="flex items-center gap-2 justify-center">
                  <Clock className="w-4 h-4 text-dental-gold" />
                  <span>{isRTL ? 'مدة العلاج: ' : 'Treatment Duration: '}{caseItem.duration[language]}</span>
                </li>
                <li className="flex items-center gap-2 justify-center">
                  <CheckCircle className="w-4 h-4 text-dental-gold" />
                  <span>{isRTL ? 'تقييم المريض: ' : 'Patient Rating: '}{caseItem.rating}/5</span>
                </li>
              </ul>
            <div 
              className='flex flex-row gap-2 mt-6 items-center justify-center align-center '>
              <Button
                className="w-full bg-gradient-to-r from-dental-gold to-dental-darkGold text-white hover:shadow-lg rounded-full py-2 text-sm font-semibold transition-all duration-200 hover:scale-105"
                asChild
              >
                <Link to="/appointment">{isRTL ? 'احجز استشارتك الآن' : 'Book Your Consultation'}</Link>
              </Button>
              <Button
                className=" w-full bg-green-600 text-white hover:bg-green-700 rounded-full py-2 text-sm font-semibold flex items-center justify-center gap-2 transition-all duration-200 hover:scale-105"
                asChild
              >
                <a href="https://wa.me/+201001234567" target="_blank" rel="noopener noreferrer">
                  <FaWhatsapp className="w-4 h-4" />
          ...
          <div id="before-after" className="flex items-center justify-center w-full ">
       
            <BeforeAfterSlider
              beforeImage={caseItem.beforeImage}
              afterImage={caseItem.afterImage}
              altText={caseItem.title[language]}
            />
          </div>
        </motion.div>
      </section>
      ...
      {/* CTA Section */}
      <motion.section
        className="py-12 bg-gradient-to-r from-dental-gold/5 to-dental-darkGold/5 text-white"
        variants={containerVariants}
        initial="hidden"
        animate="visible"
      >
        <div className="container mx-auto px-4 sm:px-6 lg:px-8 text-center max-w-4xl">
          ...
          <div className={cn('flex flex-col sm:flex-row gap-3 justify-center', isRTL ? 'flex-row' : '')}>
            <Button
              size="lg"
              className="bg-white text-dental-gold hover:text-dental-gold rounded-full px-6 py-2 text-sm font-semibold flex items-center gap-2 transition-all duration-200 hover:scale-105"
              asChild
            >
              <Link to="/appointment">
            ...
            <Button
              size="lg"
              variant="outline"
              className="bg-transparent text-white  hover:text-white rounded-full px-6 py-2 text-sm font-semibold flex items-center gap-2 transition-all duration-200 hover:scale-105"
              asChild
            >
              <a href="https://wa.me/+201001234567" target="_blank" rel="noopener noreferrer">
                <FaWhatsapp className="w-4 h-4" />
                ...
                >
                  <Link to={`/case/${item.id}`}>
                    <Card className="relative overflow-hidden rounded-xl shadow-md hover:shadow-lg transition-shadow duration-300 border border-dental-gold/10">
                      <LazyImage
                        src={item.imageSrc}
                        alt={`${item.title[language]} - عيادة د. محمد خشبة`}
                        className="w-full h-56 object-cover transition-transform duration-500 group-hover:scale-105"
                        loading="lazy"
                      />
                      <div
                        className={cn(
                          'absolute bottom-0 w-full p-3 bg-gradient-to-t from-dental-dark/70 to-transparent text-white',
